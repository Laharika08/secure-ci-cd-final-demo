...
module "ecs" {
  ...
  services = {
    "prehire-service" = {
      service_name = "prehire-service"
      image        = "${var.account_id}.dkr.ecr.us-east-1.amazonaws.com/${terraform.workspace}:latest"
      port         = 3000
      cpu          = 256
      memory       = 512
      environment = [
        ...
        {
          "name" : "DB_HOST",
          "value" : "${terraform.workspace}-db.cy4m9otgjtzc.us-east-1.rds.amazonaws.com"
        },
        {
          "name" : "DB_NAME",
          "value" : "prehire"
